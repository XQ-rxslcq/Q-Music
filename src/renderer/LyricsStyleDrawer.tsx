...
function patchFont(value: ThemeSettings, lang: LyricLang, font: string): ThemeSettings {
...
function patchPageFont(value: ThemeSettings, lang: LyricLang, font: string): ThemeSettings {
...
export default function LyricsStyleDrawer({ open, value, onChange, onClose }: Props) {
  const [page, setPage] = useState<StylePage>('desktop')

  useEffect(() => {
    if (open) setPage('desktop')
  }, [open])
  ...
  return (
    <DrawerShell
      ...
      title="歌词样式"
      onClose={onClose}
      maskClassName="form-mask"
      panelClassName="theme-drawer form-dialog lyrics-style-dialog"
    >
      <div className="form-pager" role="tablist" aria-label="歌词样式分页">
        <button
          type="button"
          role="tab"
          aria-selected={page === 'desktop'}
          className={page === 'desktop' ? 'is-active' : ''}
        ...
        <button
          type="button"
          role="tab"
          aria-selected={page === 'page'}
          className={page === 'page' ? 'is-active' : ''}
      ...
      {page === 'desktop' ? (
        <div className="form-page">
          <p className="theme-tip">
            播放条「词」可开关桌面歌词；解锁后可拖缩放。本页仅改桌面浮层样式。
          </p>
          ...
          <h3 className="theme-sub">按语种字体</h3>
              ...
              <div className="lm-num-row" style={{ marginTop: 8 }}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    onChange(
                      patchDesktop(value, {
                        strokeWidth: Math.max(1, Number(e.target.value) || 2),
                      }),
                    )
                  }
                />
          ...
          {dl.karaoke !== false ? (
            <>
              <div className="theme-field">
                ...
                <span className="theme-tip">仅当前句内渐变；下一句用下方「其它句」色。</span>
              </div>
              <div className="theme-field">
                <span>其它句（下一句）</span>
                <div className="lm-num-row">
          ...
          <label className="theme-field">
            ...
            <input
              ...
              className="ui-range"
              onChange={(e) =>
                onChange(patchDesktop(value, { opacity: Number(e.target.value) / 100 }))
              }
            />
          </label>
          <label className="theme-field">
          ...
          <div className="form-page-nav">
            <button type="button" className="primary" onClick={() => setPage('page')}>
              下一页：歌曲详情页 →
            </button>
          </div>
        </div>
        ...
        <div className="form-page">
          ...
          <label className="theme-field">
            ...
            />
            <span className="theme-tip">0 为实底遮罩；大于 0 时毛玻璃透出下层界面。</span>
          </label>
          <label className="theme-field">
            <span>长句滚动</span>
            <div className="seg">
          ...
          <div className="form-page-nav">
            <button type="button" onClick={() => setPage('desktop')}>
              ← 上一页：桌面歌词
            </button>
          </div>
        </div>
